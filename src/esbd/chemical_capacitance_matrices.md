...
(And in fact in ideal cases we often see internal chemical capacitance defined as $\mathcal{C}_{ii} = q_i^2 (\partial \mu_i / \partial c_i)^{-1}$; this definition only makes sense however when the $\mathcal{C}$ matrix is diagonal.^[Technically $\mathcal{P}_{ij} = (\partial \mu_i / \partial c_j)_{c_1,\ldots,c_N}$ is an *internal chemical elastance* matrix, and $\mathcal{C} = Q\mathcal{P}^{-1}Q$ with $Q = \mathrm{diag}(q_i)$. The diagonals of these matrices are inverses of each other only when the matrices are diagonal.])
