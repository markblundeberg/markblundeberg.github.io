...
A spectator species $j$ carries no current, so, by the blocked-ion rule from [basic transport](../basictransport/), its voltage is exactly flat, a level rail across the whole channel. The active species is the opposite. Carrying a steady current down a varying conductivity, its voltage has to slope,
...
Recall that $V^\circ_i$ is not free: it floats to wherever charge balance demands, which usually means local neutrality (the FET channel is the odd one out, a charged sheet that is neutral only jointly with its gate). With the active $V_i$ sliding and the spectator rails held flat, that balance drags $V^\circ_i$ to somewhere in between, and *how far* it follows the active carrier is decided by a competition of internal chemical capacitances, the capacitive divider from the [capacitance topic](../capacitors/), set between the active rail and the spectator rails.^[Rails coupled by the carriers' internal chemical capacitances is precisely the equivalent-circuit picture of a mixed conductor drawn by J. Jamnik and J. Maier, [Generalised equivalent circuits for mass and charge transport](https://doi.org/10.1039/b100180i), *Phys. Chem. Chem. Phys.* **3**, 1668 (2001).] When the spectators are abundant they dominate that divider and clamp $V^\circ_i$ close to their flat rails. The active carrier is then left sliding away from a nearly pinned standard state, and since
...
The semiconductor version is the neutral base of a {% wiki "bipolar junction transistor" %}. The base is flooded with majority carriers (holes, in a p-type base) that screen the field flat, so the injected minority electrons drift nowhere and simply diffuse across. The collector current saturates as soon as the collector is biased far enough to hold the electron concentration at zero on its side of the base. Transistor jargon, confusingly, reserves the word for something else.^[In BJT parlance the "saturation region" is the opposite corner of the characteristics: both junctions forward-biased, $V_{CE}$ small. What we describe here is the flat collector current of the *forward-active* region. The FET's "saturation region" in the next section does coincide with our meaning.]
