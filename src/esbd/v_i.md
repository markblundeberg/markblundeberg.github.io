...
While I *could* appoint a firm reference to nail this down, I find it actually advantageous to work reference-free.^[Traditional electrochemistry leans on a patchwork of *local* references (an SHE here, a vacuum level there), one per phase, even though there is only ever one global freedom to spend; under bias, those references can even drift apart across a single device. Working reference-free avoids the patchwork and sidesteps the perennial "but what is the *true* zero?" debates. One can instead spend the freedom on a single universal anchor, as some do with the vacuum level at infinity; but what does that buy, at this point? It merely appoints a reference that no instrument can reach. Being reference-free also has a quiet second payoff: composability. Entire devices join by simply equalizing $V_{\mathrm{e}^-}$ where wires meet, the way our multi-cell battery snapped together, and this lego construction is thermodynamically sound, unlike its vacuum-level counterpart cautioned against in [Vacuum](../vacuum/).] To actually draw a diagram we of course pin the zero somewhere; I put the negative electrode at $0$, as is usual in electronics.
