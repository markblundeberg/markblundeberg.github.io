...
In this way, the difference $\Delta$ will perfectly cancel and so the measured voltage will be equal to $V_{\mathrm{ion}}(\text{solution B}) - V_{\mathrm{ion}}(\text{solution A})$.
...
Note that $\Delta$ is related to, but distinct from, the electrode potential.^[As we will discuss, electrode potential is notionally defined as $V_{\mathrm{e}^-} - \phi_{\text{soln}}$.]
