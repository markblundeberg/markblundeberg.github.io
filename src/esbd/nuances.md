...
To reach *any* $V_i$, return to the ideal ion-selective membrane (last seen blocking a protein in [Solutions](../solutions/)), now sharpened to pass species $i$ and nothing else, charged or neutral. Thermodynamically it equilibrates to hold $V_i$ equal on its two sides.
...
{% include "esbd-diagrams/esbd-naprobe.njk" %}
{% figcaption %}
The recipe, drawn for $\mathrm{Na}^+$ with real levels. The membrane passes only $\mathrm{Na}^+$, so $V_{\mathrm{Na}^+}$ runs flat from the sample clear through to the probe's filling solution; there the saturated composition fixes $V_{\mathrm{Na}^+} - V_{\mathrm{Cl}^-}$ and the silver chloride electrode fixes $V_{\mathrm{e}^-} - V_{\mathrm{Cl}^-}$, so the wire's $V_{\mathrm{e}^-}$ sits a calibrated constant above the sample's $V_{\mathrm{Na}^+}$, with no liquid junction anywhere in the chain. The wire is our ground, and the whole probe holds still with it: slide the sample concentration and only the sample's *other* levels move. (Slide it all the way up and the sample matches the filling solution, and every level runs flat across.)
{% endfigcaption %}
</figure>

## Repurposing ion-selective electrodes

This is closely related to the {%wiki "ion-selective electrode" %} (ISE), normally described as a way to measure the activity of ion $i$; the pH meter's glass electrode is the family's household name. I would rather reinterpret it in the $V_i$ framework, where it is more honest to say an ISE *accesses* $V_i$.
