...
Choices, choices! It's complicated, but that is just how electrochemical thermodynamics is. In pure mathematical thermodynamics we don't have to make these choices; everything can stay in covariant, convention-free terms. But a diagram has to draw actual lines, so we decide.
