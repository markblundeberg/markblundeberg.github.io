...
**A spatially varying $E_{\mathrm{eq}}$.** The Nernst equation still gives the reaction's equilibrium level, but under load that level is no longer a single number: it varies through the solution, both because concentrations vary (concentration polarization) and because $V^\circ$ itself slopes (the ohmic drop). $E_{\mathrm{eq}}$ has become a field — drawn out, along with the overpotential step itself, just below.
