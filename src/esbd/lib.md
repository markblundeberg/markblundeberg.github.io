...
Both the negative (anode) and positive (cathode) electrodes in a lithium-ion battery are typically {%wiki "Intercalation (chemistry)", "intercalation electrodes" %}, where mobile $\mathrm{Li}^+$ ions may enter and mingle with mobile electrons. With both carriers coexisting in the one material, an electrode acts as a capacitor strung straight between its $V_{\mathrm{Li}^+}$ and $V_{\mathrm{e}^-}$ rails, the [ambipolar chemical capacitance](../v_i/) we just met. Charging it means intercalating more lithium, banking a $\mathrm{Li}^+$ and an $\mathrm{e}^-$ together, with every bit stored sliding the gap $V_{\mathrm{Li}^+} - V_{\mathrm{e}^-}$. Unlike the fixed steps at the earlier electrodes, this gap is no material constant: it grows as lithium loads into the host and depends on the host material itself, so the step $V_{\mathrm{e}^-} - V_{\mathrm{Li}^+}$ slides with the state of charge. That sliding gap is just the chemical capacitance being nonlinear.
