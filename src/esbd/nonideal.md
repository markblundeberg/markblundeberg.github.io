...
Among these, the famous **TATB hypothesis** is worth singling out: a trace of the tetraphenylarsonium/tetraphenylborate twin ions dissolves in any electrolyte however concentrated, and declaring them equal activity coefficients there would settle the ambiguity just as the MacInnes convention does. But its proponents do not seem to go that far. The hypothesis's actual habitat is one level up, splitting standard-state *transfer* energies between different solvents, with within-solution activity corrections extrapolated away before it is invoked — and there is a physical reason to stop there. The twins' designed symmetry is against the *solvent*; in a concentrated electrolyte each twin instead faces its own counterparty (the matrix's anions for one, its cations for the other), and no charge-flip symmetry protects that. Tetraphenylborate famously precipitates potassium; tetraphenylarsonium has no analogous quarrel with chloride. The twins, and the candidate real $\phi$'s generally, get their own treatment in [$\phi$ under the microscope](../phi/).

The first two conventions generally agree for semi-dilute solutions where the main form of nonideality comes from the long-range ionic atmosphere effects, which are indeed symmetrical for ions of $\pm z$ charge. But the conventions do diverge as we move beyond dilute solutions. Quantitatively, the disagreements in $a_i$ might amount to perhaps tens of percent between different conventions, and tens of millivolts in various potentials. This might be disastrous (in precision metrology) or negligible (in battery research). Anyway, regardless of which convention is chosen, and regardless of the accuracy of their motivations, the convention choice has no impact on actual observable results.
