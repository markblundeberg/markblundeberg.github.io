...
You might expect the famous **TATB hypothesis** to appear in this list, and logically it could: a trace of the tetraphenylarsonium/tetraphenylborate twin ions dissolves in any electrolyte however concentrated, and declaring them equal activity coefficients there would settle the ambiguity just as the MacInnes convention does. But its proponents do not seem to go that far. The hypothesis's actual habitat is one level up, splitting standard-state *transfer* energies between different solvents, with within-solution activity corrections extrapolated away before it is invoked — and there is a physical reason to stop there. The twins' designed symmetry is against the *solvent*; in a concentrated electrolyte each twin instead faces its own counterparty (the matrix's anions for one, its cations for the other), and no charge-flip symmetry protects that. Tetraphenylborate famously precipitates potassium; tetraphenylarsonium has no analogous quarrel with chloride. The twins get their own treatment in [$\phi$ under the microscope](../phi/).
