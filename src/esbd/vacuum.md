...
For ions there is no such collapse. Each ionic species parks at its own level in vacuum, offset from $\phi_{\mathrm{vac}}$ by a gas-phase formation energy that our chemical-potential conventions have already fixed: on the voltage axis, a gaseous $\mathrm{Na}^+$ sits near $\phi_{\mathrm{vac}} + 6\ \mathrm{V}$.^[From $V = \phi_{\mathrm{vac}} + \Delta_{\mathrm f}G^\circ/(z_i F)$ with $\Delta_{\mathrm f}G^\circ(\mathrm{Na}^+\!,\mathrm{g}) \approx 574$ kJ/mol. Fine print: the tabulated $\Delta_{\mathrm f}G^\circ$ places the ion at the gas *standard state* (an ideal gas at 1 bar) under the tables' [electron conventions](https://webbook.nist.gov/chemistry/ion/#TC), so this rung is a gas-phase standard-state level $V^\circ_{\mathrm{Na}^+}(\mathrm{g})$, while the electron's rung is welded to $\phi_{\mathrm{vac}}$ as a rest energy. Converting a rung between rest and standard-state placements shifts it by thermal terms ($\approx 0.4$ V for $\mathrm{Na}^+$ at room temperature), and the electron conventions shuffle a further few hundredths of a volt. One more illustration that rung placement is bookkeeping.] So "the vacuum level" is really a family, one rung per species, and it collapses onto a single line only for electrons, and only by convention. The idea of work function generalizes accordingly: each species has a well-defined $W_i$, the gap from its in-material $V_i$ up to *its own* vacuum rung.^[The combination $\alpha_i = \bar\mu_i - z_i F \psi$ is sometimes called the "real chemical potential", and the generalized work function is $W_i = \varepsilon_{i,0} - \alpha_i$, where $\varepsilon_{i,0}$ is the species' conventional rest energy in vacuum.] That gap is the physically meaningful piece; the placement of the rungs is bookkeeping.

<figure class="demo-container" style="max-width: 480px">
{% include "esbd-diagrams/esbd-vac-rungs.njk" %}
{% figcaption %}
...
> Suppose we have two uncharged materials floating in vacuum some distance apart. Since they are uncharged, $\phi_\mathrm{vac}$ is flat in between them and equal to $\psi$ of both bodies. We then bring the materials closer and closer together until their $\psi$ levels *fuse together*. From the contact assumption, we then derive the charge transfer, the degree of semiconductor {% wiki "band bending" %}, or a solution's {% wiki "diffuse layer" %}.

As argued by R. Tung,^[R. T. Tung, [The physics and chemistry of the Schottky barrier height](https://doi.org/10.1063/1.4858400), *Appl. Phys. Rev.* **1**, 011304 (2014).] the vacuum contact thought experiment basically supposes that the two materials come together without any interaction, in a weird unphysical sort of superposition. What actually happens when materials meet is a detailed chemical bonding process that depends quite sensitively on microscopic details and the specific identities of all atoms involved. In *all* cases there is some kind of atomic-scale rearrangement of atoms, bonds, and charges right at the interface, creating an interface-specific double layer that cannot be predicted from macroscopic observables ($\psi$ included).
...
The famous "absolute" electrode potential belongs on this list. The widely-quoted $4.44\ \mathrm{V}$ for the standard hydrogen electrode^[From Farrell & McTigue's measurements as interpreted in the IUPAC recommendation behind the number: S. Trasatti, [The absolute electrode potential: an explanatory note](https://doi.org/10.1351/pac198658070955), *Pure Appl. Chem.* **58**, 955 (1986). The procedure quietly splits a mean activity coefficient evenly between the ions to locate "the SHE." See also van Hees & Zhang, [doi:10.1021/acs.jpclett.4c02923](https://doi.org/10.1021/acs.jpclett.4c02923), on the link to an "ionic work function."] is best read as an electrode's work function: a legitimate, useful *surface* property, drifting with surface preparation and contamination like any other work function, and no more a universal zero than any other rung. [The reference-electrodes topic](../references/) shows where it sits on a cell diagram.
